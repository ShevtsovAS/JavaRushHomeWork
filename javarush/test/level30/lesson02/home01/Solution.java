...
//        if (!isValidated(number)) throw new NumberFormatException();
//        if (number.getNumerationSystem().equals(expectedNumerationSystem)) return number;
    ...
    private static boolean isValidated(Number number) {
        int max = number.getNumerationSystem().getNumerationSystemIntValue() - 1;
        return number.getDigit().matches(String.format("[0-%d]+", max));
    }
}